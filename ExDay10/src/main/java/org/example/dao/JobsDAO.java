...
public class JobsDAO {
    ...
    //private static final String SELECT_JOB_WITH_MIN_PAGINATION = "select * from jobs wher min_salary =? order by Job_id limit ? offset ?";
    ...
    public void deletjob(int job_id) throws SQLException, ClassNotFoundException {
        Class.forName("org.sqlite.JDBC");
        Connection conn = DriverManager.getConnection(url);
...
//        return jobs;
//    }
    public ArrayList<jobs> selectAllJobs(Double min_salary) throws SQLException, ClassNotFoundException {
        Class.forName("org.sqlite.JDBC");
        Connection conn = DriverManager.getConnection(url);
        PreparedStatement st;
         ...
         if(min_salary != null) {
            ...
            st = conn.prepareStatement(SELECT_ALL_JOB);

        }
        ResultSet rs = st.executeQuery();
        ArrayList<jobs> j = new ArrayList<>();
        while (rs.next()) {
            j.add(new jobs(rs));
        }
